...
const connection = sql.createConnection({
  ...
  port: process.env.HOLDINGS_PORT,
...
server.delete("/api/deleteCoin", function (req, res) {
  console.log(req);
  connection.query(
    `DELETE FROM cryptoholdings WHERE coin = '${req.body.coin}';`,
    (error, data) => {
      if (error) {
        console.error();
      } else {
...
server.post("/api/addOrUpdateCoin", function (req, res) {
  connection.query(
    `INSERT INTO cryptoholdings (coin, amount) VALUES ('${req.body.coin}', '${req.body.amount}') ON DUPLICATE KEY UPDATE amount = '${req.body.amount}'`,
    (error, data) => {
      if (error) {
        console.error();
      } else {
