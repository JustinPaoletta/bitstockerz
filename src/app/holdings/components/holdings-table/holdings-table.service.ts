...
export class HoldingsTableService {
  ...
  getPricingInfo(stash: Coin[]): Observable<CoinData[]> {
    return forkJoin(
      stash.map(
        (crypto: Coin) => { 
          return this.http.get<CoinData>(`https://api.coingecko.com/api/v3/coins/${crypto.coin}`) 
        }
      )
    ).pipe(
      map((prices: CoinData[]) => { 
        return prices.map((price: CoinData) => {
          stash.map((crypto: Coin) => {
            if (crypto.coin === price.id) {
              ...
              price.total_value = price.market_data.current_price.usd * Number(price.amount_owned);
            }
          })
          return price
        }) 
  ...
  deleteCoin(coin: string): Observable<SQLResponse> {
    return this.http.delete<SQLResponse>(`http://18.190.141.163/api/deleteCoin`, { body: {coin} });
  }
  ...
  calculateTotalCrypto(pricingData: CoinData[]): Observable<number> {
    ...
    pricingData.forEach((price: CoinData) => {
        totalCrypto += Number(price.amount_owned) * price.market_data.current_price.usd;
    });
