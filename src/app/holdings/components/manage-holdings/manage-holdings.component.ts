...
export class ManageHoldingsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private manageHoldingsService: ManageHoldingsService,
    private holdingsTableService: HoldingsTableService
  ) {}

  ngOnInit(): void {
    this.heldCoinsSub$ = this.manageHoldingsService
      .getHeldCoins()
      .subscribe((coinsHeld: string[]) => {
        this.coinsHeld = coinsHeld;
      });
    this.supportedCoinsSub$ = this.manageHoldingsService
      .getSupportedCoins()
      .subscribe((supportedCoins: string[]) => {
  ...
  addOrUpdateCoin(): void {
    if (Number(this.amountEntered)) {
      this.addOrUpdateCoinSub$ = this.manageHoldingsService
        .addOrUpdateCoin(this.selected, this.amountEntered)
        .pipe(
          switchMap(() => {
        ...
        .subscribe((coinData: CoinData[]) => {
          ...
          this.validInputMessage = '';
        });
    } else {
      this.validInputMessage = 'The value entered must be a valid integer';
    }
  }
  ...
  filterSupportedCoins(val: string) {
    this.supportedCoins = this.supportedCoinsCopy.filter((coin) => {
      if (coin.indexOf(val.toLowerCase()) > -1) {
        return coin;
      }
      return;
    });
