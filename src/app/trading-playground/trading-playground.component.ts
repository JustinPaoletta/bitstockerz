...
export class TradingPlaygroundComponent implements OnInit, OnDestroy {
  ...
  supportedCoins: Observable<MarketCap100[]> =
    this.store$.select(selectCoinData);
  ...
  getAllCoins(): void {
    this.coinDataSub$ = this.tradingsService
      .getCoinData()
      .pipe(take(1))
  ...
  getSingleCoinInfo(): void {
    ...
    this.http
      .get(
        `https://api.coingecko.com/api/v3/coins/${this.selected}/market_chart/range?vs_currency=usd&from=${past}&to=${now}`
      )
      .subscribe((data: any) => {
    ...
    this.coinByIdSub$ = this.tradingsService
      .getCoinById(this.selected)
