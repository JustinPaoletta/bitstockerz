...
export class CryptoNewsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.cryptoNewsSub$ = this.cryptoNews.getCryptoNews().subscribe((news: NewsArticle[]) => {
      news.forEach((article: NewsArticle) => {
        article.created_at = new Date(Date.parse(article.created_at)).toLocaleTimeString();
      });
